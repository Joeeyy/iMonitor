...
class UDPServerConnection: Connection {
    ...
    func createSocketWithAddressFamilyFromAddress(address: String) -> Bool {
        ...
        newResponseSource.setCancelHandler() {
            testVPNLog("closing udp socket for connection \(self.identifier)")
        ...
        newResponseSource.setEventHandler() {
            ...
            guard bytesRead >= 0 else {
                ...
                testVPNLog("recvfrom failed: \(errorString)")
                self.closeConnection(.all)
                return
            }
            
            guard bytesRead > 0 else {
                testVPNLog("recvfrom returned EOF")
                self.closeConnection(.all)
                return
            }
            ...
            testVPNLog("UDP connection id \(self.identifier) received = \(bytesRead) bytes from host = \(endpoint.host) port = \(endpoint.port)")
    ...
    override func sendDataWithEndPoint(_ data: Data, host: String, port: Int) {
        ...
        if responseSource == nil {
            guard createSocketWithAddressFamilyFromAddress(address: host) else {
                testVPNLog("UDP ServerConnection initialization failed.")
                return
            }
        }
        ...
        case AF_INET:
            ...
            guard serverAddress.setFromString(host) else {
                testVPNLog("Failed to convert \(host) into an IPv4 address")
                return
            }
        ...
        case AF_INET6:
            ...
            guard serverAddress.setFromString(host) else {
                testVPNLog("Failed to convert \(host) into an IPv6 address")
                return
            }
            serverAddress.setPort(port)
        ...
        guard sent > 0 else {
            let errorString = String(cString: strerror(errno))
            testVPNLog("UDP connection id \(identifier) failed to send data to host = \(host) port \(port). error = \(errorString)")
        ...
        if sent == data.count {
            // Success
            testVPNLog("UDP connection id \(identifier) sent \(data.count) bytes to host = \(host) port \(port)")
        }
    }
